...
func (a *CommonAPI) NetPeerSync(ctx context.Context, peers string) error {
	log.Infof("net peers: %s", peers)
	addrs := strings.Split(peers, "\n")
	//pis, err := addrutil.ParseAddresses(ctx, addrs)
	//if err != nil {
	//	return err
	//}

	for _, addr := range addrs {
		if len(addr) > 0 && !strings.Contains(addr, a.Host.ID().String()) {
			var p peer.AddrInfo
			err := p.UnmarshalJSON([]byte(addr))
			if err != nil {
				log.Errorf("unmarshal [%s] failed: %s", addr, err.Error())
			}
			...
			err = a.Host.Connect(ctx, p)
			if err != nil {
				log.Errorf("conn to [%s] failed: %s", p.ID.String(), err.Error())
			}
		}
	}
...
func (a *CommonAPI) NetFindPeer(ctx context.Context, p peer.ID) (peer.AddrInfo, error) {
	...
	// return peer.AddrInfo{},xerrors.New("not implement")
}
